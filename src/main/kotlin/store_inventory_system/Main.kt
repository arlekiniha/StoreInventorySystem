...
class MainLoop(
    ...
    fun start() {
        val store = Store(transactionManager)
    ...
    private fun testFlow(store: Store) {
        store.addProduct(
            InventoryItem("apple", 5.0, 15, transactionManager)
        )
        store.addProduct(
            InventoryItem("pear", 3.0, 20, transactionManager)
        )
    ...
    private fun addItemToCatalog(store: Store) {
        ...
        store.addProduct(InventoryItem(name, price, stock, transactionManager))
...
fun main() {
    ...
    val loop = MainLoop(transactionManager)
    loop.start()
}
